...
extern PetscErrorCode noiseGeneration(DM, Vec*, void* );
            ...
            for (i=xstart; i<xstart+xdimension; i++) {
                ...
                for (l=0; l<data.Ndof; l++){
                    ...
                    for ( l=0; l<data.Ndof; l++) {
                        ...
                        f[k][j][i].f[l]=data.gamma*(uxx+uyy+uzz)-data.gamma*(data.mass+data.lambda*phisquare)*ucentral;
                }
            }
        }
    }
...
//Noise Genration
 ...
 PetscErrorCode noiseGeneration( Vec* finalnoise, void* ptr)
{
    global_data     *user=(global_data*) ptr;
    model_data      data=user->model;
